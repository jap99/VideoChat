...
func removeCurrentUserFromGroup(group: NSDictionary) { // get hold of users
    ...
    if (group[kOWNERID] as? String)! == backendless!.userService.currentUser.objectId as String {   // if user is the owner
...
func updateMembersInRecent(members: [String], group: NSDictionary) {    // gets all the recents for specific chatroom
    firebase.child(kRECENT).queryOrdered(byChild: kCHATROOMID).queryEqual(toValue: (group[kGROUPID] as? String)!).observeSingleEvent(of: .value) { (snapshot) in
        if snapshot.exists() {      // go through each recent that belongs to same chatroom
            for recent in ((snapshot.value as! NSDictionary).allValues as Array) {      // update recent group members
...
func updateRecentGroupMembers(members: [String], recent: [String: AnyObject]) {     // creating value of new members and saving it
...
func deleteChatroom(chatRoomID: String) { // our chatroom is under kMESSAGE
